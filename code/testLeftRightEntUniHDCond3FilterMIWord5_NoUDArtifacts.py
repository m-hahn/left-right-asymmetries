...
content_deps = ["acl", "advcl", "advmod", "amod", "ccomp", "csubj", "obj", "iobj", "nmod", "nsubj", "nummod",  "xcomp"]
function_deps = ["aux", "lifted_case", "lifted_cop", "lifted_mark"]

content_deps = content_deps + function_deps 
   ...
   for line in linearized: # they already come in linear order
      ...
      if line["coarse_dep"] != "root":
        ...
        if reordering[line["head"]] > line["reordered_index"]:
           currentlyCrossing.append((reordering[line["head"]], line["reordered_index"], line["coarse_dep"], "L", reordering[line["head"]]))
#        else:
...
vocab_size = len(itos_state)

decoder = nn.Linear(128,vocab_size).cuda()
...
#           print("stateInput", stateInput.size())
           ...
           word_softmax = word_softmax.view(-1, vocab_size)

  #         print(word_softmax.size(), crossingIndices[1:].size())
           loss = loss_op(word_softmax.view(-1, vocab_size), crossingIndices[1:].contiguous().view(-1)).view(-1, batchSize).sum()
