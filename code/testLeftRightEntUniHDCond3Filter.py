...
dependencyCounterTotal = 0.0


#totalProb = 0
#for left in conditionalCountsLR:
#   for right in conditionalCountsRL:
##q      totalProb += exp(log(max(conditionalCountsLR[left].get(right,0.0)-delta, 0.0) + delta * (conditionalCountsRL[right]['_TOTAL_']/dependencyCount) * (len(conditionalCountsLR[left])-1)) - log(conditionalCountsLR[left]['_TOTAL_']) + log(conditionalCountsLR[left]['_TOTAL_']) - log(dependencyCount))
#      totalProb +=  exp(log(max(conditionalCountsRL[right].get(left, 0.0)-delta, 0.0) + delta * (conditionalCountsLR[left]['_TOTAL_']/dependencyCount) * (len(conditionalCountsRL[right])-1)) - log(conditionalCountsRL[right]['_TOTAL_'])+ log(conditionalCountsRL[right]['_TOTAL_']) - log(dependencyCount))
#
#print totalProb
#quit()
...
for sentence in corpus.iterator():
  ...
  for line in sentence:
    ...
    left = (depPOS if DH else headPOS)
    right = (depPOS if not DH else headPOS)
    partLR = log(max(conditionalCountsLR[left][right]-delta, 0.0) + delta * (conditionalCountsRL[right]['_TOTAL_']/dependencyCount) * (len(conditionalCountsLR[left])-1)) - log(conditionalCountsLR[left]['_TOTAL_']) + log(conditionalCountsLR[left]['_TOTAL_']) - log(dependencyCount)
    partRL = log(max(conditionalCountsRL[right][left]-delta, 0.0) + delta * (conditionalCountsLR[left]['_TOTAL_']/dependencyCount) * (len(conditionalCountsRL[right])-1)) - log(conditionalCountsRL[right]['_TOTAL_'])+ log(conditionalCountsRL[right]['_TOTAL_']) - log(dependencyCount)
    if DH:
...
for left, entry in conditionalCountsLR.iteritems():
   entL -= entry['_TOTAL_'] * (log(entry['_TOTAL_']) - log(dependencyCounterTotal))
for right, entry in conditionalCountsRL.iteritems():
   entR -= entry['_TOTAL_'] * (log(entry['_TOTAL_']) - log(dependencyCounterTotal))
print [entL/dependencyCounterTotal, entR/dependencyCounterTotal]

# the left side is `more informative about dependencies'

# compute the MI between left and right and dependency type
