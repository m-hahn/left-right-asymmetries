...
library(tidyr)
library(lme4)
data = data %>% filter(LR < 0.1) # 0.1 was the first learning rate I tried, but it leads to bad losses on some of the larger corpora

summary(lmer(Entropy ~ Model + (1|Language), data=data %>% filter(Type == "MIWord5_NoUDArtifacts.py")))
