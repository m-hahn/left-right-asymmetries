...
for sentence in corpus.iterator():
  ...
  for line in sentence:
    ...
    left = (depPOS if DH else headPOS)
    right = (depPOS if not DH else headPOS)
    surpLR -= (log(max(conditionalCountsLR[left][right]-delta, 0.0) + delta * (conditionalCountsRL[right]['_TOTAL_']/dependencyCount) * (len(conditionalCountsLR[left])-1)) - log(conditionalCountsLR[left]['_TOTAL_']))
    surpRL -= (log(max(conditionalCountsRL[right][left]-delta, 0.0) + delta * (conditionalCountsLR[left]['_TOTAL_']/dependencyCount) * (len(conditionalCountsRL[right])-1)) - log(conditionalCountsRL[right]['_TOTAL_']))
  assert surpLR > 0
  assert surpRL > 0
